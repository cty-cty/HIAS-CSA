...
const activeSpectrum = autumn.find((c) => c.name === '主被动光谱探测技术');
assert.equal(activeSpectrum?.hours, '50');
assert.equal(activeSpectrum?.credits, 2.5);
const network = autumn.find((c) => c.name === '计算机网络技术');
const database = autumn.find((c) => c.name === '高级数据库系统');
const materialSynthesis = autumn.find((c) => c.name === '材料合成与制备（材料与化工）');
assert.equal(network?.schedules?.[0]?.day, '周三');
assert.equal(database?.schedules?.[0]?.day, '周二');
assert.equal(materialSynthesis?.schedules?.[0]?.weeksText, '第3-5,7-17周');
...
assert.equal(courseOpportunity({name:'自然语言处理'}),'fall_only');
...
assert.equal(phd.publicRequiredDegreeCredits,11);
assert.equal(phd.totalCredits,38);
assert.equal(phd.degreeCourseCredits,16);
assert.equal(phd.publicElectiveCredits,2);
